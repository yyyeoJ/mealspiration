...
const Card = (props) => {
  ...
  return (
    ...
    <div className="animate-appearUp dark:bg-[#282828] bg-[#ffffff] shadow-2xl shadow-black rounded-2xl flex flex-col justify-center align-center w-[20rem] md:w-[18rem] h-full gap-2 pb-2 font-[Itim]">
      ...
      <div className="pb-2 px-2 text-lg md:text-2xl text-center border-b-2 border-white">{label}</div>
      <div className="px-2 text-sm md:text-md text-center"><span className="md:text-xl text-lg">{servings}</span> servings</div>
      <div className="px-2 text-lg md:text-xl text-center border-b-2 border-white pb-2"><span className="md:text-4xl text-3xl">{Math.floor(kcal / servings)}</span>Kcal</div>
      ...
      <div className="px-2 text-lg md:text-xl pl-10 text-left relative border-b-2 border-white pb-3" ><BsFillCircleFill className="inline  text-[#ff4b33] mr-2 text-lg"/>Fat: <span className="md:text-2xl text-xl text-right absolute right-16"><span className="md:text-2xl text-xl">{Math.floor(fat / servings)}</span>g</span></div>

      {/*Ingredients */}
      <div className="relative font-[Itim] text-lg w-full overflow-hidden border-b-2 border-white pb-2">
        ...
        {/*Content*/}
        <div className=" text-white transition-all duration-500 max-h-0 peer-checked:max-h-[100rem]">
      ...
      <div className="flex align-start items-center align-center justify-center gap-5 pt-5 pb-2">
        ...
        <button className="bg-white rounded-2xl p-2 w-16 flex justify-center h-12 md:h-10 items-center text-lg"><FaRegHeart className="text-[#a4a4a4]"/></button>
      </div>
